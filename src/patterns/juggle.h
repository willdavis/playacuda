...
void juggle() {
  ...
  byte dothue = 0;
  for(int i = 0; i < 8; i++) {
    tail_leds[beatsin16(i+7,0,SHORT_LENGTH_LEDS-1)] |= CHSV(dothue, 200, 255);
    body_leds[beatsin16(i+7,0,LONG_LENGTH_LEDS-1)] |= CHSV(dothue, 200, 255);
    lips_leds[beatsin16(i+7,0,MEDIUM_LENGTH_LEDS-1)] |= CHSV(dothue, 200, 255);
    dothue += 32;
  }
